...
class sfHMMBase(GaussianHMM):
    ...
    def model(self, s):
        if isinstance(s, str):
            ...
            raise TypeError(
                "`model` must be string or BaseStep instance, " f"but got {type(s)}"
            )
    ...
    def save(self, path: str = None, overwrite: bool = False):
        ...
        if os.path.exists(path) and not overwrite:
            raise FileExistsError(
                f"File {path} already exists. Change the name or set 'overwrite=True'."
            )
        save(self, path)
        return None
    ...
    def _hist(self, sl=None, ylim=None):
        ...
        plt.plot(
            ...
            peak_x,
            "<",
            color=self.colors["GMM"],
            markerfacecolor=self.colors["GMM marker"],
            markersize=10,
        )
        plt.hist(
            self.data_raw[sl],
            bins=n_bin,
            color=self.colors["raw data"],
    ...
    def tdp(self, **kwargs):
        ...
        axlim = (np.min(means) - np.sqrt(cov.max()), np.max(means) + np.sqrt(cov.max()))
        ...
        for sx, sy in tr:
            ...
            my = self.means_[sy]
            z += np.exp(
                -((x - mx) ** 2 / (2 * cov[sx]) + (y - my) ** 2 / (2 * cov[sy]))
            )
